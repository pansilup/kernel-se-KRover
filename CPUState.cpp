...
// bool SYCPUState::setConcreteCPUState(struct pt_regs *regs) {
...
// Jiaqi 
// bool SYCPUState::readConcreteCPUState(struct pt_regs *regs) {
...
bool SYCPUState::writeRegister(RegValue &V) {
    ...
    if (V.isSymList) {
        ...
        *R->pSymBitmap |= tmpbitmap ;
        ...
        ret = R->pSymList->Merge(V.symcellPtr) ;

    } else {
        setSYReg(R) ;
        SymCellPtr SymList (new SymCell(addr, size, V.expr)) ;
        return R->pSymList->Merge(SymList) ;
    }
...
bool SYCPUState::writeConcreteValue(MachineReg *R, RegValue &V) {
    switch (R->size) {
        case 1: {
            ...
            if((R->indx&0xf00) == (x86_64::L_REG<<8)) {
                *R->pi8 = V.i8;
            }
            else {
                // ah register;
