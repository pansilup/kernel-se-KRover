...
bool SymExecutor::process_test(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        res = oisrc1->getSymValue(e1);
        assert(res);
        //std::cout << "op1 sz : " << oisrc1->size << "op2 sz " << oisrc2->size << " \nsym expr ";
        //e1->print();
        //std::cout << "  sz " << e1->getExprSize() << std::endl;
        ...
        //std::cout << "sym expr ";
        //e1->print();
        //std::cout << "\ne1 sz: " << e1->getExprSize() << " c2 sz : " << c2->getExprSize() << std::endl;
...
bool SymExecutor::process_sbb(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    FLAG_STAT cf_bit;
    m_VM->getFlagBit(x86_64::cf, cf_bit);
...
bool SymExecutor::process_and(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        //e2->print();
        //std::cout << "  sym expr sz " << e2->getExprSize() << std::endl;
        ...
        //std::cout << "concrete expr: ";
        //c1->print();
        //std::cout << "  conc expr sz " << c1->getExprSize() << std::endl;
...
bool SymExecutor::process_or(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        assert(res);
        //pp-s
        //const expr size must tally that of the sym expr
        //ExprPtr c2(new ConstExpr(v2, oisrc2->size, 0));
        ExprPtr c2(new ConstExpr(v2, oisrc1->size, 0));
        //pp-e
...
bool SymExecutor::process_not(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    //pp-s
    //`not' is a non flag changing instruction, hence the following is not required
